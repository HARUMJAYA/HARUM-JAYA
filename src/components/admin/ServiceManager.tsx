...
import { Plus, Trash2, Loader2, Upload, ImageIcon, X, Cog } from "lucide-react";
...
const ServiceManager = () => {
  ...
  const fetchServices = async () => {
    ...
    try {
      ...
      showError("Gagal mengambil data layanan: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !category || !imageFile) {
      showError("Harap isi Judul, Kategori, dan pilih Gambar");
      return;
    }
  ...
  const handleDelete = async (id: string, imageUrl: string) => {
    ...
    try {
      ...
      if (imageUrl && imageUrl.includes('/storage/v1/object/public/services/')) {
    ...
    <div className="space-y-8">
      <Card className="border-none shadow-sm">
        <CardHeader className="bg-gray-50/50 border-b">
          <CardTitle className="text-lg flex items-center gap-2 text-gray-700">
            <Cog size={20} className="text-[#4834d4]" /> Tambah Layanan Baru
          </CardTitle>
        </CardHeader>
        <CardContent className="pt-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-gray-600">Nama Layanan</label>
                  <Input 
                    placeholder="Contoh: Jasa Konstruksi Jembatan" 
                    value={title}
              ...
              <div className="space-y-2">
                <label className="text-sm font-semibold text-gray-600">Gambar Layanan</label>
                <div className={`relative border-2 border-dashed rounded-xl min-h-[145px] flex flex-col items-center justify-center p-4 transition-all ${
                  ...
                  {previewUrl ? (
                    ...
                    <label className="cursor-pointer w-full h-full absolute inset-0 flex flex-col items-center justify-center">
                      ...
                      <p className="text-xs text-gray-500">Pilih gambar layanan</p>
                    </label>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-600">Deskripsi Singkat (Opsional)</label>
              <Textarea 
      ...
      <Card className="border-none shadow-sm">
        <CardHeader className="bg-gray-50/50 border-b flex flex-row items-center justify-between">
          <CardTitle className="text-lg text-gray-700">Daftar Layanan</CardTitle>
          <span className="text-[10px] font-bold text-gray-400 bg-white px-2 py-1 rounded border">TOTAL: {services.length}</span>
        </CardHeader>
        <CardContent className="p-0">
          {loading ? (
            <div className="flex justify-center p-12"><Loader2 className="animate-spin text-[#4834d4]" /></div>
          ) : services.length === 0 ? (
            <div className="text-center py-12 text-gray-400 italic">Belum ada layanan terdaftar.</div>
          ) : (
