...
const NewsCategory = ({ category, title }: NewsCategoryProps) => {
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-[#f8f9fa] py-16 border-b">
        <div className="container mx-auto max-w-7xl px-4">
          <h1 className="text-4xl font-bold italic uppercase text-gray-800 tracking-tight">{title}</h1>
      ...
      <main className="container mx-auto max-w-7xl px-4 py-16">
        {loading ? (
          ...
          <div className="text-center py-20 bg-gray-50 rounded-xl">
            ...
            <p className="text-gray-500 italic">Belum ada konten untuk kategori {category}.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            {news.map((item) => (
              <article key={item.id} className="group grid grid-cols-1 lg:grid-cols-5 gap-6">
                <div className="lg:col-span-2 aspect-video lg:aspect-square overflow-hidden bg-gray-100 rounded-lg shadow-sm">
                  {item.image_url ? (
